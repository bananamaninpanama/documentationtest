...
<button style="font-weight: bold" class="box" type="button" onclick="if (!(document.getElementById('dodeletepath').checked)) path = []; setkeypathurl();">Load</button>
<div style="display: flex; align-items: center;">
&emsp;<input type="checkbox" id="dodeletepath"">
<label for="dodeletepath" style="color: white; font-family: Arial; font-weight: 300; font-size: 12px;">Attempt to keep current path on load?</label>
</div>`);
...
function setkeypathurl() {
  ...
  descholder.innerHTML =
    `<label style="color: yellow; font-family: Arial;">` +
    (Array.isArray(path)
      ? path.length === 0
        ? "Select key"
        : path
